...
class Sequence(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transforms and self.mode == 'train':
            ...
            # Perform flipping horizontally with 70% probability
            ...
            # Perform flipping vertically with 70% probability
            if random.random() > 0.3:
                ...
                if 'flow_gt' in sample:
                    ...
                    sample['flow_gt'] = flow_gt
            ################################################################################################
            # Apply contrast augmentation with 50% probability
            if random.random() > 0.5:
                contrast_factor = random.uniform(0.8, 1.2)  # Contrast factor between 0.8 and 1.2
                mean = torch.mean(event_volume)
                event_volume = torch.clamp((event_volume - mean) * contrast_factor + mean, min=0, max=1)
            ################################################################################################
